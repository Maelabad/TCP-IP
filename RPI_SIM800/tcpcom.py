...
class TCPServer(Thread):
    ...
    def run(self):
        self.debug("TCPServer thread started")
        ...
        while True:
            self.debug("Calling blocking accept()...")
            conn, self.addr = self.serverSocket.accept()
            if self.terminateServer:
                self.conn = conn
                break
            if self.isClientConnected:
                self.debug("Returning from blocking accept(). Client refused")
                try:
        ...
        self.isServerRunning = False
        self.debug("TCPServer thread terminated")
    ...
    def terminate(self):
        ...
        '''
        self.debug("Calling terminate()")
        if not self.isServerRunning:
            self.debug("Server not running")
            return
        self.terminateServer = True
        self.debug("Disconnect by a dummy connection...")
    ...
    def disconnect(self):
        ...
        the LISTENING state
        '''
        self.debug("Calling Server.disconnect()")
        if self.isClientConnected:
            self.isClientConnected = False
            try:
                self.stateChanged(TCPServer.LISTENING, str(self.port))
                ...
                print("Caught exception in TCPServer.LISTENING:", e)
            self.debug("Shutdown socket now")
            try:
    ...
    def sendMessage(self, msg):
        ...
        '''
        self.debug("sendMessage() with msg: " + msg)
        ...
        try:
            ...
            self.debug("Exception in sendMessage()")
...
class ServerHandler(Thread):
    ...
    def run(self):
        self.debug("ServerHandler started")
        bufSize = 4096
        try:
            while True:
                data = ""
                ...
                isRunning = True
                while not reply[-1:] == "\0":
                    self.debug("Calling blocking conn.recv()")
                    reply = self.server.conn.recv(bufSize)
                    if reply == None or len(reply) == 0:  # Client disconnected
                        self.debug("conn.recv() returned None")
                        isRunning = False
                        break
                    data += reply
                if not isRunning:
                    break
                self.debug("Received msg: " + data + " len: " + str(len(data)))
            ...
            self.debug("Exception from blocking conn.recv(), Msg: " + str(sys.exc_info()[0]) + \
                       " at line # " + str(sys.exc_info()[-1].tb_lineno))

        self.server.disconnect()
        self.debug("ServerHandler terminated")
...
class TCPClient:
    ...
    def sendMessage(self, msg, responseTime=0):
        ...
        self.debug("sendMessage() with msg = " + msg)
        if not self.isClientConnected:
            self.debug("sendMessage(): Connection closed.")
            return None
        reply = None
        try:
            msg += "\0";  # Append \0
    ...
    def _waitForReply(self, responseTime):
        self.debug("Calling _waitForReply()")
        ...
        if self.receiverResponse == None:
            self.debug("Timeout while waiting for reply")
        else:
            self.debug("Response = " + self.receiverResponse + " time elapsed: " + str(
    ...
    def isConnected(self):
        '''
        Returns True of client is connected to the server.
...
class ClientHandler(Thread):
    ...
    def run(self):
        self.client.debug("ClientHandler thread started")
        while True:
            try:
                ...
                self.client.debug("Exception in readResponse() Msg: " + str(sys.exc_info()[0]) + \
                                  " at line # " + str(sys.exc_info()[-1].tb_lineno))
        ...
        self.client.debug("ClientHandler thread terminated")

    def readResponse(self):
        self.client.debug("Calling readResponse")
        bufSize = 4096
        data = ""
        while not data[-1:] == "\0":
            try:
                ...
                if len(reply) == 0:
                    self.client.debug("recv returns null length")
                    raise Exception("recv returns null length")
            except:
                self.client.debug("Exception from blocking conn.recv(), Msg: " + str(sys.exc_info()[0]) + \
                                  " at line # " + str(sys.exc_info()[-1].tb_lineno))
